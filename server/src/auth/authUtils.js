...
const decodeTokens = async (clientId, tokenInput) => {

    try {
        const tokens = await findTokenById(clientId);
        if (!tokens) {
            throw new UnauthorizeError('Invalid clientId')
        }


        const data = await verifyJWT(tokenInput, tokens.privateKey)
        return data
    } catch (error) {
        console.log("error decode tokens:", error);
    }
